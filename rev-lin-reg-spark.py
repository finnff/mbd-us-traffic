...
def process_file(file_path: str):
    ...
    traffic_df = traffic_df.withColumn(
        "EndTime", F.to_timestamp("EndTime", "yyyy-MM-dd'T'HH:mm:ss.SSSXXX")
    )
    traffic_df = traffic_df.withColumn(
    ...
    traffic_df = traffic_df.filter(
        (col("TrafficJamDuration(mins)") > 0) & (col("TrafficJamDuration(mins)") <= 1440)
    )  # Filter valid TrafficJamDuration values (greater than 0 and max 24 hours)

    # Map Congestion_Speed strings to numeric values using category_map
    category_map = {"fast": 60.0, "moderate": 40.0, "slow": 20.0}
    ...
    traffic_df = traffic_df.withColumn(
        "SpeedNumeric", map_congestion_speed(F.col("Congestion_Speed"))
    )
    ...
    traffic_df = (
        traffic_df.withColumn("GoodWeather", good_weather_udf(F.col("Weather_Conditions")))
...
weather_vars_reversed = list(reversed(weather_vars))
traffic_vars_reversed = list(reversed(traffic_vars))
weather_vars = weather_vars_reversed
traffic_vars = traffic_vars_reversed
max_predictors = 2  # Define the max number of predictors
results = []
data = "/user/s3521281/filled_traffic"  # on hdfs cluster use: /user/s3521281/filled_traffic
output_file = "REVregression_results_log.csv"
...
try:
    with open(output_file, "r", newline="") as csvfile:
        ...
        for row in reader:
            existing_experiments.add(
                (row[0], row[6])
            )  # target and formula uniquely identify an experiment
    ...
    for n in range(1, max_predictors + 1):
        for predictors in combinations(weather_vars, n):
            ...
            if assembled_df.count() < 2:
                print(
                    f"CP: Insufficient data for {traffic_target} with predictors {predictors_list}"
                )
                continue
            ...
            try:
                ...
                significant_features = [
                    (predictors_list[i], coeff) for i, coeff in enumerate(coefficients) if abs(coeff) > 1e-8
                ]
                num_significant_features = len(significant_features)
              
                # Format the statistics to 4 decimals
                r2_formatted = f"{r2_value:.4f}"
                ...
                print(
                    f"CP: Model: {formula_str} | R²: {r2_formatted} | "
                    f"Significant Vars: {num_significant_features}"
                )
            except Exception as e:
                print(
                    f"CP: Failed for {traffic_target} with predictors {predictors_list}: {str(e)}"
                )
